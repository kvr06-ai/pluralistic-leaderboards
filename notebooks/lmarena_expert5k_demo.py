"""Sensitivity-check reproduction on the arena-expert-5k dataset.

Mirrors :mod:`notebooks.lmarena_demo` but targets the smaller, expert-only
slice ``lmarena-ai/arena-expert-5k`` (last updated 2025-11-05). Same
algorithms, same hyperparameters, same dispersion grid; only the data loader
and output filenames differ. Used to verify that the qualitative findings
(Algorithm 3 cuts max gamma_hat at low phi, retains stability where
Algorithm 2 fails at high phi) hold on more recent expert-vote data.

Outputs (relative to repository root):
    results/results-expert-5k.json
    results/run_log.md  (appended manually, not by this script)
    results/fig_stability_vs_bt-expert-5k.png
    results/fig_mallows_clusters-expert-5k.png
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # arena-expert-5k has ~5k rows total, dropping ties typically leaves
    # ~3-4k. Default to no cap; keep the flag for parity with lmarena_demo.py.
...
def main() -> None:
    ...
    t0 = time.time()
    print(
        f"[1/5] Loading arena-expert-5k (n_models={args.n_models}, "
        f"max_battles={args.max_battles}) ..."
    )
    sample = load_lmarena_expert5k(
    ...
    results: dict = {
        "config": {
            "dataset": "lmarena-ai/arena-expert-5k",
            "dataset_last_updated": "2025-11-05",
    ...
    _plot_stability_curves(
        per_phi_curves,
        args.phis,
        args.out_dir / "fig_stability_vs_bt-expert-5k.png",
    )
    _plot_mallows_clusters(
        centers,
        weights,
        cat_names,
        sample.model_names,
        args.out_dir / "fig_mallows_clusters-expert-5k.png",
    )
    results_path = args.out_dir / "results-expert-5k.json"
...
def _plot_stability_curves(per_phi: dict, phis: list, path: Path) -> None:
    ...
    for ax, phi in zip(axes, phis):
        ...
        ax.set_title(f"Ranking stability (phi = {phi})  --  arena-expert-5k")
...
def _plot_mallows_clusters(
    ...
    ax.set_title("Per-category Bradley-Terry central rankings (arena-expert-5k; lower = better)")
